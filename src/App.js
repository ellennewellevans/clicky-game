...
class App extends Component {
  ...
  shuffleScoreCard = id => {
    let clickedPrincessIds = this.state.clickedPuppyIds;

    if(clickedPrincessIds.includes(id)){
      this.setState({ clickedPrincessIdss: [], score: 0, status:  "You Lost! Click to play again." });
      return;
    }else{
      clickedPrincessIds.push(id)

      if(clickedPrincessIds.length === 8){
        this.setState({score: 8, status: "You Won! Click to play again.", clickedPrincessIds: []});
        console.log('You Win');
        return;
      }

      this.setState({ princesses, clickedPrincessIds, score: clickedPrincessIds.length, status: " " });
    ...
    return (
      <div className="App">
        ...
        <ScoreCard total={this.state.score}
               ...
               />
        <Wrapper>
          {this.state.princesses.map(puppy => (
            <CharacterCards
